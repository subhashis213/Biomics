...
router.get('/users/:username/insights', authenticateToken('admin'), async (req, res) => {
  try {
    ...
    const [
      ...
      QuizAttempt.find({ username }).sort({ submittedAt: -1 }).limit(200).lean(),
      TopicTestAttempt.find({ username }).sort({ submittedAt: -1 }).limit(200).lean(),
      FullMockAttempt.find({ username }).sort({ submittedAt: -1 }).limit(200).lean(),
      MockExamAttempt.find({ username }).sort({ submittedAt: -1 }).limit(200).lean(),

...
export default function AdminCourseWorkspacePage() {
  ...
  async function handleRenameCourse() {
    const nextName = String(renameCourseValue || '').trim();
    if (!selectedCourse || !nextName) return;
    if (normalizeKey(nextName) === normalizeKey(selectedCourse)) {
      setModalMessage({ type: 'error', text: 'Please enter a different course name.' });
      return;
    }
    setRenamingCourse(true);
    try {
      const response = await renameCourseAdmin(selectedCourse, nextName);
      const nextCourses = Array.isArray(response?.courses) ? response.courses : [];
      setCourseCatalog(nextCourses);
      const renamed = nextCourses.find((entry) => normalizeKey(entry?.name) === normalizeKey(nextName));
      const resolvedName = renamed?.name || nextName;
      setSelectedCourse(resolvedName);
      setSelectedBatchByCourse((current) => {
        const next = { ...current };
        const oldKey = selectedCourse;
        if (Object.prototype.hasOwnProperty.call(next, oldKey)) {
          next[resolvedName] = next[oldKey];
          delete next[oldKey];
        }
        return next;
      });
      setRenameCourseValue('');
      setModalMessage({ type: 'success', text: `Course renamed to "${resolvedName}". Updated across workspace content.` });
    } catch (error) {
      setModalMessage({ type: 'error', text: error.message || 'Failed to rename course.' });
    } finally {
  ...
  async function handleRenameBatch(oldBatchName) {
    const nextBatchName = String(renameBatchValue || '').trim();
    if (!selectedCourse || !oldBatchName || !nextBatchName) return;
    if (normalizeKey(oldBatchName) === normalizeKey(nextBatchName)) {
      setModalMessage({ type: 'error', text: 'Please enter a different batch name.' });
      return;
    }
    setRenamingBatch(true);
    try {
      const response = await renameCourseBatchAdmin(selectedCourse, oldBatchName, nextBatchName);
      const nextCourses = Array.isArray(response?.courses) ? response.courses : [];
      const nextCourse = nextCourses.find((entry) => normalizeKey(entry?.name) === normalizeKey(selectedCourse));
      const nextBatch = (nextCourse?.batches || []).find((entry) => normalizeKey(entry?.name) === normalizeKey(nextBatchName));
      setCourseCatalog(nextCourses);
      setSelectedBatchByCourse((current) => ({
        ...current,
        [selectedCourse]: nextBatch?.name || nextBatchName
      }));
      setRenamingBatchName('');
      setRenameBatchValue('');
      setModalMessage({ type: 'success', text: `Batch renamed to "${nextBatch?.name || nextBatchName}". Updated across content sections.` });
    } catch (error) {
  ...
  return (
    ...
    >
      <section id="builder-workspace" className="card admin-course-workspace" style={courseModalStyle}>
        ...
        <section className="card course-setup-panel">
          <div className="section-header compact">
            <div>
              <p className="eyebrow">Course Setup</p>
              <h3>Create, rename, and delete courses + batches</h3>
            </div>
          </div>

          <div className="course-setup-grid">
            <article className="course-setup-card">
              <h4>Courses</h4>
              ...
              <div className="course-setup-row">
                <input
                  type="text"
                  placeholder="Create new course (e.g. CSIR NET)"
                  value={newCourseName}
              ...
              <div className="course-setup-row">
                <input
                  type="text"
                  placeholder="Rename selected course"
                  value={renameCourseValue}
                  onChange={(event) => setRenameCourseValue(event.target.value)}
                  disabled={!selectedCourse || renamingCourse}
                />
                <button type="button" className="secondary-btn" onClick={handleRenameCourse} disabled={!selectedCourse || !renameCourseValue.trim() || renamingCourse}>
                  {renamingCourse ? 'Renaming...' : 'Rename Course'}
                </button>
                <button type="button" className="danger-btn" onClick={handleDeleteSelectedCourse} disabled={!selectedCourse || deletingEntity}>
                  Delete Course
                </button>
              </div>
              ...
              <div className="course-chip-list">
                {courseCatalog.length ? courseCatalog.map((entry) => (
                  <button
                    key={`course-chip-${entry.name}`}
                    type="button"
                    className={`course-chip${normalizeKey(selectedCourse) === normalizeKey(entry.name) ? ' active' : ''}`}
                ...
                )) : <p className="empty-note">No courses yet.</p>}
              </div>
            </article>

            <article className="course-setup-card">
              <h4>Batches {selectedCourse ? `for ${selectedCourse}` : ''}</h4>
              <div className="course-setup-row">
                <select
                  value={selectedBatch}
                  onChange={(event) => {
              ...
              >
                <input
                  type="text"
                  placeholder="Create batch (e.g. Batch 2027)"
                  value={newBatchName}
              ...
              <div className="course-setup-row">
                <input
                  type="text"
                  placeholder="Rename selected batch"
                  value={renamingBatchName ? renameBatchValue : ''}
                  onChange={(event) => setRenameBatchValue(event.target.value)}
                  disabled={!selectedCourse || !selectedBatch || renamingBatch}
                  onFocus={() => {
                    if (!renamingBatchName && selectedBatch) {
                      setRenamingBatchName(selectedBatch);
                      setRenameBatchValue(selectedBatch);
                    }
                  }}
                />
                <button
                  ...
                  onClick={() => {
                    const target = renamingBatchName || selectedBatch;
                    if (!target) return;
                    handleRenameBatch(target);
                  }}
                  disabled={!selectedCourse || !selectedBatch || !renameBatchValue.trim() || renamingBatch}
                >
                  {renamingBatch ? 'Renaming...' : 'Rename Batch'}
                </button>
                <button type="button" className="danger-btn" onClick={() => setDeletePrompt({ type: 'batch', target: selectedBatch })} disabled={!selectedCourse || !selectedBatch || savingBatch}>
                  Delete Batch
                </button>
              </div>
              ...
              {selectedCourse ? (
                <div className="course-chip-list">
                  {availableBatches.length ? availableBatches.map((batchName) => (
                    <button
                      key={`batch-chip-${batchName}`}
                      type="button"
                      className={`course-chip${normalizeKey(batchName) === normalizeKey(selectedBatch) ? ' active' : ''}`}
                      onClick={() => {
                        setSelectedBatchByCourse((current) => ({ ...current, [selectedCourse]: batchName }));
                        setRenamingBatchName(batchName);
                        setRenameBatchValue(batchName);
                      }}
                      title={batchName}
                    >
                      {batchName}
        ...
        {(workspaceMode === 'full' || workspaceMode === 'module-topic') ? (
          <section className="course-context-bar">
            <div className="course-context-row">
              <label className="course-context-field">
                <span>Course</span>
                <select
                  value={selectedCourse}
                  onChange={(event) => {
                ...
                >
                  {!courseCatalog.length ? <option value="">No course available</option> : null}
                  {courseCatalog.map((entry) => (
                    <option key={`module-topic-course-${entry.name}`} value={entry.name}>
                      {entry.displayName || entry.name}
                    </option>
                  ))}
                </select>
              ...
              <label className="course-context-field">
                ...
                >
                  {!availableBatches.length ? <option value="">No batch</option> : null}
                  {availableBatches.map((batchName) => (
                    <option key={`module-topic-batch-${batchName}`} value={batchName}>
                      {batchName}
                    </option>
                  ))}
                </select>

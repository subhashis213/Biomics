...
export default function WorkingPremiumVideoCard({
  video,
  adminMode,
  onDeleteVideo,
  onFileSelect,
      ...
      .then((YT) => {
        ...
        playerRef.current = new YT.Player(ytTarget, {
          ...
          playerVars: { 
            rel: 0, 
            modestbranding: 1, 
            autoplay: 1, // Enable autoplay to start video immediately
            controls: 1, // Show YouTube controls for debugging
          ...
          events: {
            ...
            onStateChange: (event) => {
              ...
              if (state === YTState.PAUSED || state === YTState.BUFFERING) {
                console.log('Video paused/buffering');
                setIsPlaying(false);
  ...
  function handleWatchResume() {
    console.log('Watch/Resume clicked');
  ...
  function handleHideVideo() {
    console.log('Hide Video clicked');
    clearSaveInterval();
    clearControlsTimeout();
    persistCurrentPlayback();
    if (isPlayerReady && playerRef.current) {
  ...
  return (
    <article className={`compact-premium-video-card ${adminMode ? 'compact-premium-video-card-admin' : ''} ${isPlaying && isPlayerOpen ? 'playing' : ''}`}>
      {/* Only show thumbnail when player is not open */}
      {!isPlayerOpen && <VideoThumbnail url={video.url} title={video.title} />}
      
      {/* Player shell - only visible when player is open */}
      {canPlayInline && (
